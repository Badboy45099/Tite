-- Anti-Duplication Check: Destroy previous running instances before executing
if getgenv().TeleportScriptLoaded then
    -- Clean up previous ScreenGuis from CoreGui
    for _, gui in ipairs(game:GetService("CoreGui"):GetChildren()) do
        if gui.Name == "TpDraggableGui" or gui.Name == "Fluent" then
            gui:Destroy()
        end
    end
    -- Clean up previous 3D World Beacons
...
-- Helper: Load JSON File from Executor Workspace
...
-- Helper: Save JSON File to Executor Workspace
...
-- FLOATING TP BUTTON ACTION (Strict Category Listener)
IconBtn.MouseButton1Click:Connect(function()
    local myHRP = GetHRP(LocalPlayer)
    if not myHRP then return end

    -- Mode 1: Saved position
    ...
    elseif CurrentCategoryMode == "Random" then
        ...
        -- Check lock mode condition
        if LockModeEnabled and LockedPlayer then
            target = LockedPlayer
        else
            -- If lock mode is off, pick a random player when clicked
            local validPlayers = {}
...
local Window = Fluent:CreateWindow({
    ...
    SubTitle = "Categorized UI Controller",
    TabWidth = 140,
...
Tabs.Main:AddToggle("TpBtnToggle", {
...
local CategoryDropdown = Tabs.Main:AddDropdown("CategoryDropdown", {
...
----------------------------------------------------
local SavedSection = Tabs.Main:AddSection("Saved Positions")
local RandomSection = Tabs.Main:AddSection("Random Player Teleport")
local SpecificSection = Tabs.Main:AddSection("Specific Player Teleport")
...
SavedSection:AddButton({
    ...
    Callback = function()
        ...
        -- Reset state selection and update the UI dropdown
        SelectedSaveName = ""
        savedFilesDropdown:SetValues(GetSavedListKeys())
        
        -- Clear any active 3D world pins
